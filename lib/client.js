...
.dsw-bal-cur{color:var(--dsw-alias-label-secondary);font-size:11px;line-height:14px;display:inline-flex;align-items:flex-start;gap:5px;}
.dsw-band{display:inline-flex;align-items:center;justify-content:center;padding:0 6px;height:14px;border-radius:999px;font-size:10px;line-height:1;font-weight:600;flex:none;white-space:nowrap;}
      ...
      return h('div', { className: 'dsw-dock' },
        ...
        h('div', { className: 'dsw-bal-row' },
          h('span', { className: 'dsw-bal-cur' },
            '本会话消耗',
            cost && cost.ok === true && cost.band ? h('span', { className: 'dsw-band ' + (cost.band === 'peak' ? 'peak' : cost.band === 'offPeak' ? 'offpeak' : 'base') }, cost.band === 'peak' ? '高峰价' : cost.band === 'offPeak' ? '空闲价' : '基础价') : null,
          ),
        ...
        open && view && view.balances.length ? h('div', { className: 'dsw-detail' },
          view.balances.map((b) => h('div', { key: b.currency, className: 'dsw-detail-row' },
            h('span', { className: 'dsw-detail-label' }, b.currency + ' 赠金 / 充值'),
            h('span', { className: 'dsw-detail-val' }, fmt(Number(b.granted_balance)) + ' / ' + fmt(Number(b.topped_up_balance))),
          )),
        ) : null,
        ...
        h('div', { className: 'dsw-dock-btns' },
          ...
          h('button', { type: 'button', className: 'dsw-btn', onClick: () => setOpen((o) => !o) }, open ? '收起' : '明细'),
        ),
      );
    }
