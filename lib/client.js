...
.dsw-threshold-symbol{color:var(--dsw-alias-label-secondary);font-size:11px;}
.dsw-threshold-input{width:54px;background:var(--dsw-alias-bg-layer-1);border:1px solid var(--dsw-alias-border-l1);color:var(--dsw-alias-label-primary);border-radius:6px;padding:2px 6px;font-size:11px;font-family:inherit;text-align:right;font-variant-numeric:tabular-nums;box-sizing:border-box;}
.dsw-threshold-input:focus{outline:none;border-color:var(--dsw-alias-brand-primary);}
.dsw-center{text-align:center;line-height:1.6;}
...
`;
    ...
    function WalletDock(props) {
      ...
      const threshold = view && view.costThreshold !== undefined ? view.costThreshold : 5;
      ...
      const saveThreshold = (val) => {
        ...
        fetch('/wallet/api/set-threshold', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ threshold: t }) })
          .then((r) => r.json())
          .then((v) => { if (v && v.ok) { apiGet('/wallet/api/balance').then((b) => setView(b), () => {}); } })
          .catch(() => {});
      };

      const statusLine = !view ? '连接中…'
        : (view.error ? '余额查询失败'
          : (view.available ? '上下文长度正常' : '账户不可用'));
      ...
      return h('div', { className: 'dsw-dock' },
        ...
        overThreshold
          ? h('div', { className: 'dsw-err dsw-center' },
              h('div', null, '⚠ 当前窗口上下文过长'),
              h('div', null, '请尝试新建对话避免余额浪费'))
          : h('div', { className: 'dsw-hint dsw-center' }, statusLine),
