...
export function apply(ctx) {
  ...
  let balanceAt = 0;
  let inflight = null;
  const costThresholds = new Map(); // 每个会话独立的消耗上限（默认 5 元）
  ...
  registerRoute("GET", "/wallet/api/cost", async (req) => {
    ...
    return { ok: true, costThreshold: costThresholds.get(sid) ?? 5, ...sessionCost(session) };
  });
  registerRoute("POST", "/wallet/api/set-threshold", async (req) => {
    ...
    if (sid && Number.isFinite(t) && t >= 0) costThresholds.set(sid, t);
    return { ok: true, costThreshold: sid ? (costThresholds.get(sid) ?? 5) : 5 };
  });
